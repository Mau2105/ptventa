...
class GenerateSale extends Component
{
    ...
    public $product_amount = 0; // Contiene la cantidad del producto seleccionado
    ...
    public function updatedProductAmount($value){
        if($value > 0 && !empty($this->product_id)){
            $this->product_subtotal = priceFormat($value * revertPriceFormat($this->product_price));
        } else {
            $this->product_subtotal = null;
        }
    ...
    public function addProduct() {
        if(empty($this->product_id) || $this->product_amount <= 0){
            return;
        }
        $inventory = $this->inventoryProduct($this->product_id);
        $product_amount_selected = $this->selected_products->where('product_element_id', $this->product_id)->sum('product_amount');
    ...
    public function verifySelectedProduct(){
        $this->change_value = $this->payment_value ? revertPriceFormat($this->payment_value) - $this->total : 0; // Recalcular el valor de cambio
