...
<li class="breadcrumb-item">
    <a href="{{ route('ptventa.'.getRoleRouteName(Route::currentRouteName()).'.reports.index') }}" class="text-decoration-none">
                ...
                <div class="row mb-3 align-items-end">
                    ...
                    <div class="col-md d-flex gap-2">
                        @if(Auth::user()->havePermission('ptventa.'.getRoleRouteName(Route::currentRouteName()).'.reports.generate.sales.pdf'))
                            <form action="{{ route('ptventa.'.getRoleRouteName(Route::currentRouteName()).'.reports.generate.sales.pdf') }}"
                                  method="post" target="_blank" class="m-0">
                                @csrf
                                <input type="hidden" name="start_date" value="{{ $start_date }}">
                                <input type="hidden" name="end_date" value="{{ $end_date }}">
                                <button type="submit" class="btn btn-danger">
                                    {{ trans('ptventa::reports.Btn_Generate_Sales_Report') }}
                        ...
                        @if(Auth::user()->havePermission('ptventa.'.getRoleRouteName(Route::currentRouteName()).'.reports.generate.products.pdf'))
                            <form action="{{ route('ptventa.'.getRoleRouteName(Route::currentRouteName()).'.reports.generate.products.pdf') }}"
                                  method="post" target="_blank" class="m-0">
                                @csrf
                                <input type="hidden" name="start_date" value="{{ $start_date }}">
                                <input type="hidden" name="end_date" value="{{ $end_date }}">
                                <button type="submit" class="btn btn-outline-secondary">
                                    {{ trans('ptventa::reports.Btn_Generate_Products_Report') }}
                                        ...
                                        <tr>
                                            @if ($index === 0)
                                                ...
                                                <td rowspan="{{ count($movement->movement_details) }}" style="vertical-align: middle;">
                                                    {{ optional($movement->movement_responsibilities->where('role', 'CLIENTE')->first())->person->full_name ?? 'N/A' }}
                                                </td>
                                                <td rowspan="{{ count($movement->movement_details) }}" style="vertical-align: middle;">{{ $movement->registration_date }}</td>
                                            @endif
                                            <td>{{ $movement_detail->inventory->element->name ?? $movement_detail->inventory->element->product_name }}</td>
...
Toast.fire({ icon: 'info', title: '{{ trans('ptventa::reports.Title') }}' })
</script>
@endpush